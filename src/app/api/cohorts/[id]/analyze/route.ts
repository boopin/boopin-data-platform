  ...
  try {
    ...
    for (const [cohortPeriod, visitorSet] of Array.from(cohortGroups.entries())) {
      ...
      for (const period of retentionPeriods) {
        // Get activity for these visitors within the retention window
    ...
    const limitedCohortAnalysis = cohortAnalysis.slice(0, 12);
    ...
    return NextResponse.json({
      cohort: {
        ...
        interval_type: cohort.interval_type,
        retention_periods: retentionPeriods
      },
      analysis: limitedCohortAnalysis,
